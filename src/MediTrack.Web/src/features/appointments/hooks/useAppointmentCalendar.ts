  ...
  const { calendar, setEvents } = useCalendar({
    config: {
      ...
      callbacks: {
        ...
        // DnD rescheduling: ScheduleX fires onEventUpdate when an event is moved.
        // @schedule-x/drag-and-drop v4 is not yet available (only v3 exists).
        // When a v4-compatible DnD plugin is released, add it to the plugins array
        // and this callback will handle the confirmation flow.
