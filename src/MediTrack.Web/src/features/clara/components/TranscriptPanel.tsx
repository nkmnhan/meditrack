...
import type { TranscriptLine, VoiceCommand, PendingTranscriptLine } from "../types";
...
interface TranscriptPanelProps {
  ...
  readonly pendingLine?: PendingTranscriptLine | null;
}
      ...
      >
        {lines.length === 0 && !pendingLine ? (
          ...
          <>
            ...
            {pendingLine && (
              <div className="flex gap-3 items-start rounded-md px-2 py-1 -mx-2 opacity-60">
                <span
                  className={clsxMerge(
                    "inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium flex-shrink-0",
                    pendingLine.speaker === "Doctor"
                ...
                <p className="flex-1 text-sm text-foreground/60 italic leading-relaxed break-words">
                  {pendingLine.text}
                </p>
              </div>
            )}
...
function VoiceWaveform({ size = "sm" }: VoiceWaveformProps) {
  const barClass = size === "lg"
    ? "waveform-bar w-1.5 rounded-full bg-accent-500"
    : "waveform-bar w-1 rounded-full bg-accent-500";
